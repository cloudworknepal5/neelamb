/**
 * Name: counter.js (The Universal Fix)
 * Features: Multi-function Digit Converter, Object-to-Number Parser, Refresh Guard
 */
...
const BloggerCounter = {
    ...
    updateAndFetch: async function(elementId) {
        ...
        // सेसन चेक (रिफ्रेस हुँदा संख्या बढ्न नदिन)
        ...
        try {
            ...
            // --- [OBJECT OBJECT] निवारण ---
            let count = 0;
            if (data !== null) {
                if (typeof data === 'number') {
                    count = data;
                } else if (typeof data === 'object') {
                    // यदि डेटाबेसमा {value: 1} जस्तो अब्जेक्ट छ भने त्यसबाट १ निकाल्ने
                    count = parseInt(Object.values(data)[0]) || 0;
                } else {
                    count = parseInt(data) || 0;
                }
            }

            // यदि नयाँ भिजिटर हो भने १ थप्ने र डेटाबेसमा पठाउने
            if (!hasVisited) {
                count += 1;
                await fetch(url, {
                    method: 'PUT',
                    body: JSON.stringify(count) // केवल शुद्ध नम्बर मात्र पठाउने
                });
                sessionStorage.setItem("v_" + postId, "true");
            }

            // नतिजा देखाउने (आइकन सहित)
            if (el) {
