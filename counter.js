/**
 * Updated & Robust Multi-function Visitor Counter
 * Fix: Data Validation and Force Integer Conversion
 */
const BloggerCounter = {
    ...
    updateAndFetch: async function(elementId) {
        ...
        try {
            ...
            // डेटा कसरी आएको छ चेक गर्ने (Important Fix)
            let count = 0;
            if (data !== null) {
                if (typeof data === 'number') count = data;
                else if (typeof data === 'object' && data.value) count = parseInt(data.value);
                else if (typeof data === 'string') count = parseInt(data);
            }

            let newCount = count + 1;

            // डेटाबेसमा सिधै नम्बर मात्र पठाउने
            await fetch(url, {
                method: 'PUT',
            ...
            if (el) {
                const nepNum = this.toNepali(newCount);
                ...
                el.innerHTML = `${eyeIcon} <span>${nepNum} पटक हेरियो</span>`;
            }
        } catch (error) {
            console.error("Counter Error:", error);
            if (el) el.innerText = "Error loading views";
        }
    }
};
